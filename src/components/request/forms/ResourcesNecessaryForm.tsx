// src/components/request/forms/ResourcesNecessaryForm.tsx
...
const ResourcesNecessaryForm: React.FC<ResourcesNecessaryFormProps> = ({ resources, setResources }) => {
  ...
  const updateResource = useCallback((id: number, field: keyof Omit<IResource, 'id'>, value: string | number) => {
    setResources(prev =>
      prev.map(res => (res.id === id ? { ...res, [field]: value } : res))
    )
  }, [setResources])

  const handleNumberChange = useCallback((id: number, field: 'quantity' | 'quantityDay', e: ChangeEvent<HTMLInputElement>) => {
    const value = parseFloat(e.target.value)
    updateResource(id, field, isNaN(value) ? 0 : value)
  }, [updateResource])

  const deleteResource = useCallback((id: number, index: number) => {
    if (index === 0) return
        ...
        {resources.map((res, index) => (
          <div key={res.id} className="resource">
            ...
            <input
              ...
              type="number"
              name="quantityDay"
              value={res.quantityDay}
              onChange={(e) => handleNumberChange(res.id, 'quantityDay', e)}
            />

            {/* Exibe o botão de exclusão somente para os registros adicionais */}
            {index > 0 && (
              <button
                type="button"
